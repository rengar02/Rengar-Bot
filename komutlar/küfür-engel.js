...
message.channel.send(new MessageEmbed().setDescription(`${message.author} Küfür korumasını başarıyla \`açtın\`.`).setColor('0x348f36').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
return
}
if (args[0] === 'kapat') {
...
message.channel.send(new MessageEmbed().setDescription(`${message.author} Küfür korumasını başarıyla \`kapattın\`.`).setColor('0x348f36').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
return
}
