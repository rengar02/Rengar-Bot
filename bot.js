const Discord = require('discord.js');// rengar
const client = new Discord.Client();// rengar
const ayarlar = require('./ayarlar.json');// rengar
const chalk = require('chalk');// rengar
const moment = require('moment');// rengar
var Jimp = require('jimp');// rengar
const { Client, Util } = require('discord.js');// rengar
const fs = require('fs');// rengar
const db = require('quick.db');// rengar
const express = require('express');//
require('./util/eventLoader.js')(client);// rengar
const path = require('path');// rengar
const snekfetch = require('snekfetch');// rengar
const ms = require('ms');// rengar
...
client.on("message", message => {
    ...
    return message.channel.send(`${message.author}, Sanada Selam hoşgeldin.`)
});

client.on("message", message => {
    if(message.content.toLowerCase() == "merhaba") 
    return message.channel.send(`${message.author}, Merhaba Kanka.`)
});
...
client.on("message", message => {
    ...
    return message.channel.send(`${message.author}, Sanada mrb.`)
});

client.on("message", message => {
    if(message.content.toLowerCase() == "slm") 
    return message.channel.send(`${message.author}, Sanada selamm hoşgeldin.`)
});
 ...
 if(!db.has(`reklam_${msg.guild.id}`)) return;
        ...
        if (reklam.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.hasPermission("BAN_MEMBERS")) {
                  msg.delete();
                    return msg.channel.send(new Discord.MessageEmbed().setDescription(`${msg.author} Bu sunucuda reklam koruması etkin.`).setColor('0x800d0d').setAuthor(msg.member.displayName, msg.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
